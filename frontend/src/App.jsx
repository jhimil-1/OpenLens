...
export default function VisualProductSearch() {
  ...
  useEffect(() => {
    const savedCollection = localStorage.getItem('visualSearchCollection');
    if (savedCollection) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('visualSearchCollection', JSON.stringify(collection));
  }, [collection]);
  ...
  const addToCollection = (product) => {
    const imageWithId = {
      ...
      collectionId: Date.now() + Math.random() // Unique ID for collection items
    };
    
    setCollection(prevCollection => [...prevCollection, imageWithId]);
