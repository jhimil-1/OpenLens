...
import torch
# open_clip import is delayed into guarded model-init block to allow tests to import this module
...
if AsyncIOMotorClient is not None:
    try:
        ...
        print(f"[MONGODB] Connection failed/skipped: {_e}")
else:
    mongo_client = None
    db = None
    collection_collection = None
    print("[MONGODB] AsyncIOMotorClient not available; skipping DB init")
...
# Initialize OpenCLIP model (guarded so tests can skip heavy init)
device = "cuda" if torch.cuda.is_available() else "cpu"
if os.getenv("SKIP_MODEL_INIT") != "1":
    try:
        # import here so tests that set SKIP_MODEL_INIT can avoid requiring the package
        import open_clip
        model, _, preprocess = open_clip.create_model_and_transforms('ViT-B-32', pretrained='openai')
        tokenizer = open_clip.get_tokenizer('ViT-B-32')
        model = model.to(device)
    except Exception as _e:
        # If model init fails at import time, set placeholders. Runtime endpoints will fail unless model is initialized.
        print(f"[WARN] OpenCLIP model init skipped/failed: {_e}")
        model = None
        preprocess = None
        tokenizer = None
else:
    # Tests can set SKIP_MODEL_INIT=1 to avoid loading the CLIP model.
    model = None
    preprocess = None
    tokenizer = None

# Initialize Qdrant Cloud client (optional)
if QdrantClient is not None:
    try:
        ...
        qdrant_client = None
        print(f"[QDRANT] Init failed/skipped: {_e}")
else:
    qdrant_client = None
...
def extract_dominant_colors(image: Image.Image, num_colors: int = 2) -> List[str]:
    ...
    ENHANCED: More precise color detection with improved black/white detection.
    """
    try:
        ...
        yy, xx = np.ogrid[:h, :w]
        cy, cx = h / 2.0, w / 2.0
        ry, rx = h * 0.40, w * 0.40  # Slightly larger mask
        ...
        # FIRST: Check the entire image for black/white dominance
        all_V = V.flatten()
        all_S = S.flatten()
        
        # Calculate average brightness and saturation
        avg_brightness = float(np.mean(all_V))
        avg_saturation = float(np.mean(all_S))
        
        print(f"  [COLOR DEBUG] Avg brightness: {avg_brightness:.3f}, Avg saturation: {avg_saturation:.3f}")
        
        # ENHANCED: Black detection (low brightness + low saturation)
        if avg_brightness < 0.25 and avg_saturation < 0.25:
            dark_pixel_ratio = float(np.mean(all_V < 0.30))
            print(f"  [COLOR DEBUG] Dark pixel ratio: {dark_pixel_ratio:.3f}")
            if dark_pixel_ratio > 0.65:
                print(f"  [COLOR DEBUG] Detected BLACK (low brightness + low saturation)")
                return ["black"]
        
        # ENHANCED: White detection (high brightness + low saturation)
        if avg_brightness > 0.80 and avg_saturation < 0.25:
            bright_pixel_ratio = float(np.mean(all_V > 0.85))
            print(f"  [COLOR DEBUG] Bright pixel ratio: {bright_pixel_ratio:.3f}")
            if bright_pixel_ratio > 0.65:
                print(f"  [COLOR DEBUG] Detected WHITE (high brightness + low saturation)")
                return ["white"]
        
        # Check center region for black/white
        center_V = V[center_mask]
        center_S = S[center_mask]
        
        if center_V.size > 0:
            center_avg_brightness = float(np.mean(center_V))
            center_avg_saturation = float(np.mean(center_S))
            
            print(f"  [COLOR DEBUG] Center avg brightness: {center_avg_brightness:.3f}, saturation: {center_avg_saturation:.3f}")
            
            # Black detection in center
            if center_avg_brightness < 0.28 and center_avg_saturation < 0.30:
                dark_ratio = float(np.mean(center_V < 0.32))
                if dark_ratio > 0.55:
                    print(f"  [COLOR DEBUG] Detected BLACK in center region")
                    return ["black"]
            
            # White detection in center
            if center_avg_brightness > 0.85 and center_avg_saturation < 0.25:
                bright_ratio = float(np.mean(center_V > 0.88))
                if bright_ratio > 0.55:
                    print(f"  [COLOR DEBUG] Detected WHITE in center region")
                    return ["white"]

        # Gray detection (medium brightness + very low saturation)
        if 0.25 < avg_brightness < 0.75 and avg_saturation < 0.15:
            gray_pixel_ratio = float(np.mean((all_V > 0.25) & (all_V < 0.75) & (all_S < 0.15)))
            if gray_pixel_ratio > 0.60:
                print(f"  [COLOR DEBUG] Detected GRAY (medium brightness + very low saturation)")
                return ["gray"]

        # Now proceed with chromatic color detection
        # ENHANCED: Only consider saturated pixels for chromatic colors
        sat_mask = S > 0.25  # Saturated pixels only
        # Allow also very bright chromatic pixels (remove overly strict upper bound)
        val_mask = V > 0.15
        keep = center_mask & sat_mask & val_mask

        if not keep.any():
            # Try without center mask
            keep = sat_mask & val_mask

        if not keep.any():
            # No chromatic colors found, determine based on brightness
            if avg_brightness < 0.35:
                print(f"  [COLOR DEBUG] No chromatic colors, defaulting to BLACK")
                return ["black"]
            elif avg_brightness > 0.75:
                print(f"  [COLOR DEBUG] No chromatic colors, defaulting to WHITE")
                return ["white"]
            else:
                print(f"  [COLOR DEBUG] No chromatic colors, defaulting to GRAY")
                return ["gray"]
    ...
    # Use a smaller background distance threshold so uniform-color images still register
    keep = keep & (dist_bg > 5.0)
        ...
        # Histogram over hue with weighted counts
        bins = np.linspace(0, hue_scale, 13, endpoint=True)
        hist, edges = np.histogram(hue, bins=bins, weights=weights)
        order = np.argsort(hist)[::-1]

        def hue_to_name(hval: float) -> str:
            """More precise hue ranges"""
            if hval < 8 or hval >= 172:
                return "red"
            if hval < 22:
                return "orange"
            if hval < 38:
        ...
        for idx in order[:max(2, num_colors + 2)]:
            ...
            hue_group = int(center_h / 20)
            if hue_group in seen_hue_ranges:
                continue

            if name not in colors and hist[idx] > hist.max() * 0.15:
                colors.append(name)
                seen_hue_ranges.add(hue_group)
                print(f"  [COLOR DEBUG] Found chromatic color: {name}")

        # If no chromatic colors detected, use brightness-based fallback
        if not colors:
            if avg_brightness < 0.35:
                return ["black"]
            elif avg_brightness > 0.75:
...
def get_color_name(r: int, g: int, b: int) -> str:
    ...
    # ENHANCED: Stricter gray detection
    if max_val - min_val < 25:  # More strict
        if brightness < 140:
            return "gray"
        else:
            return "white"
    
    # ENHANCED: More precise color detection with higher thresholds
    if r > g + 50 and r > b + 50:  # Increased from 40
        if g > b + 40:  # Increased from 30
            return "orange"
        elif r > 180:
            return "red"
        else:
            return "red"
    elif g > r + 50 and g > b + 50:  # Increased from 40
        if g > 150:
            return "green"
        else:
            return "dark green"
    elif b > r + 50 and b > g + 50:  # Increased from 40
        if b > 150 and r > 100:
            return "purple"
        elif b > 150:
            return "blue"
            ...
            return "dark blue"
    elif r > 150 and g > 150 and b < 90:  # More strict
        return "yellow"
    elif r > 150 and b > 150 and g < 90:  # More strict
        return "purple"
    elif g > 150 and b > 150 and r < 90:  # More strict
        return "teal"
...
def calculate_color_similarity(target_colors: List[str], text: str) -> float:
    """
    ENHANCED: Calculate precise color similarity score with strict matching.
    Returns a score from 0.0 to 1.0 indicating color match quality.
    """
    if not target_colors or target_colors == ["multi-color"]:
        return 1.0
    ...
    # Check for incompatible colors first
    for target_color in target_colors:
        if target_color in incompatible_colors:
            for incompatible in incompatible_colors[target_color]:
                if incompatible in text_lower:
                    print(f"    [COLOR FILTER] Rejected: Found incompatible '{incompatible}' with target '{target_color}'")
                    return 0.0
    ...
    for target_color in target_colors:
        # Exact match
        if target_color in text_lower:
            exact_matches += 1
            continue
        
        # Check variations
        variations = get_color_variations(target_color)
        for variation in variations:
            if variation in text_lower:
                partial_matches += 0.7  # Partial credit for variations
                break
    
    # Calculate score
    total_targets = len(target_colors)
    match_score = (exact_matches + partial_matches) / total_targets
...
def filter_products_by_attributes(products: List[dict], target_colors: List[str], gender: str, category: str) -> List[dict]:
    ...
    for product in products:
        ...
        if color_match and gender_match and category_match:
            # Store color score for later sorting
            product['_color_score'] = color_score
            filtered_products.append(product)
    
    # ENHANCED: Sort by color match score (higher = better match)
    filtered_products.sort(key=lambda x: x.get('_color_score', 0), reverse=True)
...
def get_color_variations(color: str) -> List[str]:
    """ENHANCED: Get variations of a color name with stricter grouping"""
    color_variations = {
        "red": ["crimson", "maroon", "burgundy", "cherry", "scarlet", "ruby", "wine"],
        "blue": ["navy", "azure", "cobalt", "sapphire", "royal blue", "denim"],
        "green": ["emerald", "forest", "olive", "lime", "mint", "jade", "sage"],
        "yellow": ["gold", "mustard", "amber", "lemon", "canary"],
        "purple": ["violet", "lavender", "magenta", "plum", "lilac", "mauve"],
        "orange": ["coral", "peach", "apricot", "tangerine", "rust"],
        "pink": ["rose", "blush", "fuchsia", "hot pink", "salmon", "dusty rose"],
        "brown": ["chocolate", "tan", "beige", "caramel", "coffee", "mocha"],
        "black": ["jet black", "ebony", "onyx", "coal"],  # REMOVED: charcoal, dark (too ambiguous)
        "white": ["ivory", "cream", "off-white", "pearl", "snow", "eggshell"],
        "gray": ["grey", "silver", "ash", "slate", "pewter"],
        "cyan": ["teal", "turquoise", "aqua"],
    }
...
def generate_search_queries(category_info: dict, colors: List[str], broad_category: str, gender: str) -> List[str]:
    ...
    # ENHANCED: Prioritize exact color + category combinations
    if colors and colors != ["multi-color"]:
        primary_color = colors[0]
        
        # Top priority: exact color + category
        queries.append(f"{primary_color} {clean_category}")
        queries.append(f"{clean_category} {primary_color}")
        
        # With gender if specified
        if gender != "unisex":
            queries.append(f"{gender} {primary_color} {clean_category}")
            queries.append(f"{primary_color} {clean_category} for {gender}")
        
        # Platform-specific with color emphasis
        platforms = ["Amazon", "Flipkart", "Myntra"]
        for platform in platforms[:2]:  # Limit to top 2 platforms
            queries.append(f"{primary_color} {clean_category} {platform}")
    
    # Base queries with gender context (lower priority)
    if gender != "unisex":
    ...
    # Fallback queries (lowest priority)
    queries.extend([
        f"{clean_category}",
    ...
    return unique_queries[:12]  # Reduced from 15 to focus on quality
...
async def search_similar_products(image: UploadFile = File(...)):
    """ENHANCED: Upload an image and find visually similar products with strict color matching"""
    try:
        try:
        ...
        print("\n" + "="*70)
        print("[VISUAL PRODUCT SEARCH - ENHANCED]")
        print("="*70)
        ...
        # ENHANCED: Override color if category name contains explicit color
        category_lower = category_info['primary_category'].lower()
        explicit_colors = {
            'black': ['black'],
            'white': ['white'],
            'red': ['red'],
        ...
        for color_keyword, color_value in explicit_colors.items():
            if color_keyword in category_lower:
                print(f"  [COLOR OVERRIDE] Category contains '{color_keyword}', overriding detected color {colors} -> {color_value}")
                colors = color_value
                break
        ...
        print(f"  Colors: {', '.join(colors)} [STRICT MATCHING ENABLED]")
        
        print(f"\n[QUERY] Generating color-focused search queries...")
        ...
        # ENHANCED: Apply STRICT attribute filtering BEFORE indexing
        print(f"\n[FILTER] Applying STRICT color-based filtering...")
        ...
        print(f"  After STRICT filtering: {len(filtered_products)} products")
        ...
        products_to_index = filtered_products if len(filtered_products) > 8 else unique_products
        ...
        # ENHANCED: Dynamic similarity threshold with color consideration
        min_threshold = 0.58  # Increased base threshold
        if category_info['broad_category'] in ['women_fashion', 'men_fashion']:
            min_threshold = 0.62  # Higher for fashion
        ...
        for hit in search_results:
            ...
            # Calculate color match score for this result
            ...
            if adjusted_similarity >= min_threshold:
                ...
                if title_lower not in seen_titles:
                    ...
                    hit.score = adjusted_similarity  # Update with adjusted score
                    filtered_results.append(hit)
        
        print(f"  After similarity + color filtering: {len(filtered_results)} products")
        ...
        print(f"  Gender: {category_info['gender']}")
        print(f"  Colors: {colors} [STRICTLY ENFORCED]")
        print("="*70 + "\n")
...
def root():
    ...
    return {
        ...
        "version": "2.0 - Enhanced Color Matching",
        "endpoints": {
        ...
        "enhancements": [
            "Strict color matching with incompatibility detection",
            "Enhanced color extraction with background suppression",
            "Color similarity scoring (0.0-1.0)",
            "Adjusted visual similarity based on color match",
            "Prioritized color-focused search queries"
        ]
    }
