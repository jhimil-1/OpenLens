...
class CollectionItemWithTryon(CollectionItem):
    """Extended collection item with try-on support"""
    can_try_on: bool = False
...
def process_image(human_img_path: str, garm_img_path: str):
    ...
    try:
        ...
        return result[0]  # Path to generated image
    except Exception as e:
    ...
    try:
        ...
        if output_path and os.path.exists(output_path):
            ...
            return FileResponse(
                ...
                headers={"Content-Disposition": "inline; filename=tryon_result.png"}
            )
        else:
            raise HTTPException(status_code=500, detail="Failed to generate try-on image")

    except Exception as e:
        print(f"[ERROR] Try-on error: {str(e)}")
        if 'human_path' in locals() and os.path.exists(human_path):
            os.unlink(human_path)
        if 'garm_path' in locals() and os.path.exists(garm_path):
            os.unlink(garm_path)
...
async def detect_pose_api(image: UploadFile = File(...)):
    ...
    try:
        ...
        return {"pose_keypoints": keypoints}

    except Exception as e:
        print(f"[ERROR] Pose detection error: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error detecting pose: {str(e)}")

@app.get("/collection/list-with-tryon", response_model=CollectionResponse)
async def get_collection_with_tryon(user_id: str = "anonymous"):
    """Get user's collection with try-on availability"""
    try:
        cursor = collection_collection.find({"user_id": user_id}).sort("created_at", -1)
        items = []
        
        async for doc in cursor:
            # Check if the item is suitable for try-on (simple check for now)
            # You can enhance this with more sophisticated checks
            can_try_on = any(ext in doc["image_url"].lower() 
                           for ext in ['.jpg', '.jpeg', '.png', '.webp'])
            
            item = CollectionItemWithTryon(
                image_url=doc["image_url"],
                collection_id=str(doc["_id"]),
                created_at=doc["created_at"],
                can_try_on=can_try_on
            )
            items.append(item)
        
        return CollectionResponse(items=items, total=len(items))
    except Exception as e:
        print(f"[ERROR] Error getting collection with try-on: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to get collection: {str(e)}")
